...
class CdcPartitionsCreator<T : Comparable<T>>(
    ...
    override suspend fun run(): List<PartitionReader> {
        ...
        val syntheticInput: DebeziumInput by lazy { dbzOps.synthesize() }
        // Ensure that the WAL position upper bound has been computed for this sync.
        val upperBound: T =
            upperBoundReference.updateAndGet { it ?: dbzOps.position(syntheticInput.state.offset) }
        // Deserialize the incumbent state value, if it exists.
        val input: DebeziumInput =
            if (incumbentOpaqueStateValue == null) {
                syntheticInput
            } else {
                dbzOps.deserialize(incumbentOpaqueStateValue, activeStreams)
            }
        // Build and return PartitionReader instance, if applicable.
        val partitionReader =
            CdcPartitionReader(concurrencyResource, outputConsumer, dbzOps, upperBound, input)
        ...
        val lowerBound: T = dbzOps.position(input.state.offset)

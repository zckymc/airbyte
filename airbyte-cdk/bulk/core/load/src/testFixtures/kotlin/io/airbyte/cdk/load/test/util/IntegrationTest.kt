...
abstract class IntegrationTest(
    ...
    fun runSync(
        ...
        check(streamStatus == AirbyteStreamStatus.COMPLETE || streamStatus == AirbyteStreamStatus.INCOMPLETE) {
            ...
            if (streamStatus != null) {
                catalog.streams.forEach {
                    val streamStatusMessage =
                        when (streamStatus) {
                            AirbyteStreamStatus.COMPLETE -> InputStreamComplete(DestinationRecordStreamComplete(it, System.currentTimeMillis()))
                            AirbyteStreamStatus.INCOMPLETE -> InputStreamIncomplete(DestinationRecordStreamIncomplete(it, System.currentTimeMillis()))
                            else -> throw IllegalStateException("Impossible: We checked that the stream status was valid at the start of this method. Somehow got $streamStatus.")
                        }
